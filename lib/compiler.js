 ...
 */
const loadTemplate = function (root, tpl) {
    let path = root + 'templates/' + tpl + '.ejs';
    if (!fs.existsSync(path)) {
        logger.error(path + ' can not be found!');
        process.exit(1);
    }

    let tplContent = fs.readFileSync(path, 'utf8');
...
exports.compileIndex = function (root, opts) {
    ...
    let tpl = loadTemplate(root, 'index');
...
exports.compilePages = function (root, opts) {
    ...
    let tpl = loadTemplate(root, 'pages');
    ...
    opts.pages.forEach(function (page) {
        let path = root + 'pages/' + page.id + '.md';
        if (!fs.existsSync(path)) {
            logger.warn(path + ' can not be found!');
            return;
        }

        let content = marked(fs.readFileSync(path, 'utf8'));
        content = content.replace(/<pre><code/g, '<pre');
        content = content.replace(/<\/code><\/pre>/g, '<\/pre>');
        opts.page = {
            title: page.title,
        ...
        let html = ejs.render(tpl, opts);
        let output = root + 'pages/' + page.id + '.html';
...
exports.compileArticles = function (root, opts) {
    ...
    let tpl = loadTemplate(root, 'articles');
    ...
    opts.articles.forEach(function (ar) {
        let pathMd = root + 'articles/' + ar.id + '.md',
        ...
        let html = ejs.render(tpl, opts);
        let output = root + 'articles/' + ar.id + '.html';
        fs.writeFileSync(output, html);
        logger.info('Build ' + output + ' completed!');
    });
};
...
exports.compile = function (root) {
    ...
    let opts = parser.parseYaml(root);
